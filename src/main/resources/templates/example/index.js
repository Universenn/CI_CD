...
async function getAll() {
    let response = await fetch("http://localhost:8080/api/v1/boards");
    if (response.ok) {
...
async function readPost(id) {
    let response = await fetch("http://localhost:8080/api/v1/boards/" + id);
    if (response.ok) {
        let data = await response.json();
        console.log(data)
        $('.board').hide();
        ...
        $('#post-contents').html(data.content);
...
function deletePost(id) {
    fetch('http://localhost:8080/api/v1/boards/' + id, {
        method: 'DELETE'
